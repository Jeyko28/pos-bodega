...
function initDB() {
  if (fs.existsSync(dbPath)) {
    ...
    data.ventas = data.ventas.map(v => ({ ...v, metodo_pago:v.metodo_pago||'Efectivo', descuento:v.descuento||0, tipo_descuento:v.tipo_descuento||'ninguno' }))
...
function completarSetup({ negocioNombre, negocioRuc, negocioDireccion, negocioTelefono, adminNombre, adminUsername, adminPassword, codigoRecuperacion }) {
  data.config = { ...data.config, negocio_nombre:negocioNombre, negocio_ruc:negocioRuc||null, negocio_direccion:negocioDireccion||null, negocio_telefono:negocioTelefono||null, ticket_mensaje:'¡Gracias por su compra! Vuelva pronto 😊' }
...
function addProducto({ nombre,precio,stock,codigo,categoria }) { const p={id:data.nextId.productos++,nombre,precio:parseFloat(precio),stock:parseInt(stock),codigo:codigo||null,categoria:categoria||'General',creado_en:new Date().toISOString()};data.productos.push(p);save();return p }
function updateProducto({ id,nombre,precio,stock,codigo,categoria }) { const i=data.productos.findIndex(p=>p.id===id);if(i!==-1){data.productos[i]={...data.productos[i],nombre,precio:parseFloat(precio),stock:parseInt(stock),codigo:codigo||null,categoria};save();return data.productos[i]} }
function deleteProducto(id) { data.productos=data.productos.filter(p=>p.id!==id);save();return{success:true} }
function reponerStock(id,cantidad) { const p=data.productos.find(p=>p.id===id);if(p){p.stock+=parseInt(cantidad);save();return{...p}} }
...
function importarProductosExcel(filas) {
  ...
  filas.forEach((f,idx)=>{
    ...
    if(codigo){const ex=data.productos.find(p=>p.codigo===codigo);if(ex){ex.nombre=nombre;ex.precio=precio;ex.stock=stock;ex.categoria=categoria;r.actualizados++;return}}
    const pn=data.productos.find(p=>p.nombre.toLowerCase()===nombre.toLowerCase())
    if(pn){pn.precio=precio;pn.stock=stock;pn.categoria=categoria;if(codigo)pn.codigo=codigo;r.actualizados++;return}
    data.productos.push({id:data.nextId.productos++,nombre,precio,stock,codigo:codigo||null,categoria,creado_en:new Date().toISOString()})
    r.agregados++
  })
...
function addCliente({nombre,telefono,referencia}) { const c={id:data.nextId.clientes++,nombre,telefono:telefono||null,referencia:referencia||null,creado_en:new Date().toISOString()};data.clientes.push(c);save();return c }
function updateCliente({id,nombre,telefono,referencia}) { const i=data.clientes.findIndex(c=>c.id===id);if(i!==-1){data.clientes[i]={...data.clientes[i],nombre,telefono:telefono||null,referencia:referencia||null};save();return data.clientes[i]} }
...
function buscarCliente(query) { const q=query.toLowerCase();return data.clientes.filter(c=>c.nombre.toLowerCase().includes(q)||(c.telefono&&c.telefono.includes(q))).map(c=>({...c,deuda_total:data.fiado.filter(f=>f.cliente_id===c.id&&f.estado==='pendiente').reduce((s,f)=>s+f.saldo,0)})) }
...
function getDatosExcel(tipo,dias) { const desde=new Date();desde.setDate(desde.getDate()-(dias||365));if(tipo==='ventas'){const us=data.usuarios;const cl=data.clientes;return data.ventas.filter(v=>new Date(v.fecha)>=desde).sort((a,b)=>new Date(b.fecha)-new Date(a.fecha)).map(v=>({'N° Venta':v.id,'Fecha':new Date(v.fecha).toLocaleDateString('es-PE'),'Hora':new Date(v.fecha).toLocaleTimeString('es-PE',{hour:'2-digit',minute:'2-digit'}),'Cajero':us.find(u=>u.id===v.usuario_id)?.nombre||'—','Cliente':cl.find(c=>c.id===v.cliente_id)?.nombre||'—','Método Pago':v.metodo_pago,'Subtotal':v.subtotal_bruto||v.total,'Descuento':v.descuento||0,'Total':v.total,'Monto Recibido':v.monto_recibido||0,'Vuelto':v.vuelto||0,'Es Fiado':v.es_fiado?'Sí':'No'}))}if(tipo==='productos'){return[...data.productos].sort((a,b)=>a.nombre.localeCompare(b.nombre)).map(p=>({'ID':p.id,'Nombre':p.nombre,'Categoría':p.categoria,'Precio':p.precio,'Stock':p.stock,'Código':p.codigo||'—','Creado':new Date(p.creado_en).toLocaleDateString('es-PE')}))}if(tipo==='clientes'){return[...data.clientes].sort((a,b)=>a.nombre.localeCompare(b.nombre)).map(c=>{const d=data.fiado.filter(f=>f.cliente_id===c.id&&f.estado==='pendiente').reduce((s,f)=>s+f.saldo,0);return{'ID':c.id,'Nombre':c.nombre,'Teléfono':c.telefono||'—','Referencia':c.referencia||'—','Deuda Total':d,'Registrado':new Date(c.creado_en).toLocaleDateString('es-PE')}})}if(tipo==='fiado'){const cl=data.clientes;return[...data.fiado].sort((a,b)=>new Date(b.fecha)-new Date(a.fecha)).map(f=>({'ID':f.id,'Cliente':cl.find(c=>c.id===f.cliente_id)?.nombre||'—','Concepto':f.concepto,'Monto Orig.':f.monto_original,'Saldo':f.saldo,'Estado':f.estado,'Fecha':new Date(f.fecha).toLocaleDateString('es-PE')}))}return[] }
...
function getHistorialVentas({ fechaDesde, fechaHasta, metodoPago, usuarioId, soloFiado, busqueda, pagina=1, porPagina=30 }) {
  ...
  // ── Usar T00:00:00 y T23:59:59 para forzar hora local (evita problema UTC en Perú) ──
  if (fechaDesde) {
    const desde = new Date(fechaDesde + 'T00:00:00')
    ventas = ventas.filter(v => new Date(v.fecha) >= desde)
  }
  if (fechaHasta) {
    const hasta = new Date(fechaHasta + 'T23:59:59')
    ventas = ventas.filter(v => new Date(v.fecha) <= hasta)
  }
  ...
  if (busqueda && busqueda.trim()) {
    ...
    ventas = ventas.filter(v => String(v.id).includes(q) || cl.find(c=>c.id===v.cliente_id)?.nombre.toLowerCase().includes(q))
  }
  ...
  return {
    ventas: ventas.slice(inicio, inicio+porPagina).map(v => ({
      ...v,
      num_productos:  data.detalle_ventas.filter(d=>d.venta_id===v.id).length,
      nombre_usuario: us.find(u=>u.id===v.usuario_id)?.nombre || '—',
      nombre_cliente: cl.find(c=>c.id===v.cliente_id)?.nombre || null,
    })),
    totalPeriodo, cantidadTotal, totalPaginas, paginaActual: pagina
  }
}

// ─── VENTAS ───────────────────────────────────────────────────────
function realizarVenta(items,montoRecibido,metodoPago='Efectivo',descuento=0,tipoDescuento='ninguno',usuarioId=null,clienteId=null,esFiado=false) {
  const subtotalBruto=items.reduce((s,i)=>s+i.subtotal,0);let descuentoMonto=0
  ...
  data.ventas.push({id:ventaId,total,subtotal_bruto:subtotalBruto,descuento:descuentoMonto,tipo_descuento:tipoDescuento,monto_recibido:esFiado?0:montoRecibido,vuelto,metodo_pago:esFiado?'Fiado':metodoPago,usuario_id:usuarioId,cliente_id:clienteId||null,es_fiado:esFiado,fecha:new Date().toISOString()})
  items.forEach(item=>{data.detalle_ventas.push({id:data.nextId.detalle_ventas++,venta_id:ventaId,producto_id:item.id,nombre_producto:item.nombre,precio_unitario:item.precio,cantidad:item.cantidad,subtotal:item.subtotal});const p=data.productos.find(p=>p.id===item.id);if(p)p.stock-=item.cantidad})
  if(esFiado&&clienteId){const concepto=items.map(i=>`${i.nombre} x${i.cantidad}`).join(', ');addFiado({clienteId,monto:total,concepto,usuarioId})}
  save();return{ventaId,total,subtotalBruto,descuentoMonto,vuelto,metodoPago:esFiado?'Fiado':metodoPago}
}
function getVentasHoy() { const hoy=new Date().toDateString();const us=data.usuarios;const cl=data.clientes;return data.ventas.filter(v=>new Date(v.fecha).toDateString()===hoy).map(v=>({...v,num_productos:data.detalle_ventas.filter(d=>d.venta_id===v.id).length,nombre_usuario:us.find(u=>u.id===v.usuario_id)?.nombre||null,nombre_cliente:cl.find(c=>c.id===v.cliente_id)?.nombre||null})).reverse() }
...
function getTopProductos(dias=7) { const desde=new Date();desde.setDate(desde.getDate()-dias);const ids=data.ventas.filter(v=>new Date(v.fecha)>=desde).map(v=>v.id);const mapa={};data.detalle_ventas.filter(d=>ids.includes(d.venta_id)).forEach(d=>{if(!mapa[d.nombre_producto])mapa[d.nombre_producto]={nombre:d.nombre_producto,cantidad:0,ingresos:0};mapa[d.nombre_producto].cantidad+=d.cantidad;mapa[d.nombre_producto].ingresos+=d.subtotal});return Object.values(mapa).sort((a,b)=>b.cantidad-a.cantidad).slice(0,8) }
