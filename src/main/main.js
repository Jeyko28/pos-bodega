...
// ─── Ventas ───────────────────────────────────────────────────────
ipcMain.handle('realizar-venta', (_, {
  items, montoRecibido, metodoPago, descuento, tipoDescuento,
  usuarioId, clienteId, esFiado,
  compradorNombre, compradorDniRuc,   // ← nuevos para SUNAT
  guardarCliente,                     // ← si guardar comprador en Clientes
}) => {
  const result = db.realizarVenta(
    items, montoRecibido, metodoPago, descuento, tipoDescuento,
    usuarioId, clienteId, esFiado,
    compradorNombre, compradorDniRuc
  )
  // Si el cajero marcó guardar cliente y no estaba registrado
  if (guardarCliente && compradorNombre && !clienteId) {
    db.addCliente({ nombre:compradorNombre, dni_ruc:compradorDniRuc||null, telefono:null, referencia:'Registrado desde caja' })
  }
  return result
})
